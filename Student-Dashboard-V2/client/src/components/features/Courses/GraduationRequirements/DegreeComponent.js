...
const DegreeComponent = ({ onSelectDegree, selectedType }) => {
    ...
    useEffect(() => {
        ...
        if (degrees.length === 0) {
            fetch(`https://student-dashboard.onrender.com:5050/graduation/degree/user/${userData.name}`)
                .then((res) => res.json())
                .then((data) => {
                    setDegrees(data);
                })
    ...
    const handleDegreeSubmit = () => {
        const url = editMode 
            ? `https://student-dashboard.onrender.com:5050/graduation/degree/${degreeToEdit._id}`
            : "https://student-dashboard.onrender.com:5050/graduation/degree";
    ...
    const handleDelete = (degreeId) => {
        fetch(`https://student-dashboard.onrender.com:5050/graduation/degree/${degreeId}`, {
            method: "DELETE",
        })
            .then((res) => res.json())
            .then((data) => {
    ...
    const updateCompletionStatus = useCallback((degree) => {
        ...
        if (degreeUpdated) {
            fetch(`https://student-dashboard.onrender.com:5050/graduation/degree/${degree._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                },
