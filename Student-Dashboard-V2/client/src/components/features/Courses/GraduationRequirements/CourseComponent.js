...
const CourseComponent = ({ selectedDegree, selectedConcentration, selectedRequirement, onCreateCourse, onSelectCourse, isRequirementDone, calculateTotalUpdates }) => {
    ...
    useEffect(() => {
        if (selectedDegree) {
            fetch(`https://student-dasboard.onrender.com/graduation/courses/${selectedDegree._id}`)
            ...
            if (hasSelectedConcentration && hasSelectedRequirement && selectedRequirement.courses.length > 0) {
                fetch(`https://student-dasboard.onrender.com/graduation/course/${selectedDegree._id}/${selectedConcentration._id}/${selectedRequirement._id}`)
                .then((response) => response.json())
                .then((data) => {
    ...
    const handleCourseSubmit = () => {
        if (hasSelectedDegree && hasSelectedConcentration && hasSelectedRequirement) {
            const url = editMode
                ? `https://student-dasboard.onrender.com/graduation/course/${selectedDegree._id}/${selectedConcentration._id}/${selectedRequirement._id}/${courseToEdit._id}`
                : `https://student-dasboard.onrender.com/graduation/course/${selectedDegree._id}/${selectedConcentration._id}/${selectedRequirement._id}`;
    ...
    const handleDelete = (courseId) => {
        fetch(`https://student-dasboard.onrender.com/graduation/course/${selectedDegree._id}/${selectedConcentration._id}/${selectedRequirement._id}/${courseId}`, {
            method: "DELETE",
        })
            .then((response) => response.json())
            .then((data) => {
    ...
    const toggleCourseCompleteness = (course) => {
        ...
        fetch(`https://student-dasboard.onrender.com/graduation/course/${selectedDegree._id}/${selectedConcentration._id}/${selectedRequirement._id}/${selectedCourse._id}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
            },
