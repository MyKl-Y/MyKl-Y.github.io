...
const RequirementComponent = ({ selectedDegree, selectedConcentration, onCreateRequirement, onSelectRequirement, calculateTotalUpdates }) => {
    ...
    useEffect(() => {
        if (hasSelectedDegree && hasSelectedConcentration && selectedConcentration.requirements.length > 0) {
            ...
            fetch(`https://student-dasboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}`)
    ...
    const handleRequirementSubmit = () => {
        if (hasSelectedConcentration && hasSelectedDegree) {
            const url = editMode
                ? `https://student-dasboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}/${requirementToEdit._id}`
                : `https://student-dasboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}`;
    ...
    const handleDelete = (requirementId) => {
        fetch(`https://student-dasboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}/${requirementId}`, {
            method: "DELETE",
        })
        .then((response) => response.json())
        .then((data) => {
