...
const RequirementComponent = ({ selectedDegree, selectedConcentration, onCreateRequirement, onSelectRequirement, calculateTotalUpdates }) => {
    ...
    useEffect(() => {
        if (hasSelectedDegree && hasSelectedConcentration && selectedConcentration.requirements.length > 0) {
            ...
            fetch(`http://https://student-dashboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}`)
    ...
    const handleRequirementSubmit = () => {
        if (hasSelectedConcentration && hasSelectedDegree) {
            const url = editMode
                ? `http://https://student-dashboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}/${requirementToEdit._id}`
                : `http://https://student-dashboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}`;
    ...
    const handleDelete = (requirementId) => {
        fetch(`http://https://student-dashboard.onrender.com/graduation/requirement/${selectedDegree._id}/${selectedConcentration._id}/${requirementId}`, {
            method: "DELETE",
        })
        .then((response) => response.json())
        .then((data) => {
