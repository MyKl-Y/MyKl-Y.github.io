...
const Authentication = () => {
    ...
    */
    ...
    const checkUsernameAvailability = async (username) => {
        if (username !== '') {
            try {
                const response = await fetch(`https://student-dashboard.onrender.com:5050/auth/check-username/${username}`);
    ...
    const checkEmailAvailability = async (email) => {
        if (email !== '') {
            try {
                const response = await fetch(`https://student-dashboard.onrender.com:5050/auth/check-email/${email.toLowerCase()}`);
                const data = await response.json()
    ...
    async function login(user) {
        try {
            const response = await fetch('https://student-dashboard.onrender.com:5050/auth/login', {
                method: 'POST',
                headers: {
    ...
    async function register(newUser) {
        try {
            const response = await fetch('https://student-dashboard.onrender.com:5050/auth/register', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
